...
import { xpexLearnerCoursesRoute, xpexPoloManagementRoute } from '@/lib/xpexRouteMap'
...
const getTeacherCourseHref = (organizationSlug: string, courseId: string) =>
  getUriWithOrg(organizationSlug, `/dash/courses/course/${courseId.replace(/^course_/, '')}/general`)
...
function PoleDashboard({ organizationSlug, data, failed }: { organizationSlug: string; data?: XpexLaunchReadinessData | null; failed: boolean }) {
  ...
  const native = (path: string) => getUriWithOrg(organizationSlug, path)
  return <>
    {data ? <XpexKpiGrid>
    ...
    <section><XpexSectionHeader eyebrow="Operação" title="Ações rápidas"/><div className="xpex-actions">
      <XpexQuickAction icon={GraduationCap} title="Nova turma" disabled={false} href={native('/dash/users/settings/usergroups')} detail="Gerenciar turmas"/>
      <XpexQuickAction icon={BookOpen} title="Novo curso" disabled={false} href={native('/dash/courses')} detail="Gerenciar cursos"/>
      <XpexQuickAction icon={Users} title="Novo aluno" disabled={false} href={native('/dash/users/settings/users')} detail="Convidar e gerenciar"/>
      <XpexQuickAction icon={FilePlus2} title="Relatórios" disabled={false} href={native('/dash/analytics')} detail="Consultar indicadores"/>
    </div></section>
