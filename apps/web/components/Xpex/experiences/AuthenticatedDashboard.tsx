...
import { xpexAdminRoute, xpexCourseStudioRoute, xpexLearnerCoursesRoute, xpexPoloCoursesRoute } from '@/lib/xpexRouteMap'
...
function PoleDashboard({ organizationName, organizationSlug, data, failed }: { organizationName?: string; organizationSlug: string; data?: XpexLaunchReadinessData | null; failed: boolean }) {
  ...
  return <>
    ...
    <section><XpexSectionHeader eyebrow="Operação" title="Ações do polo"/><div className="xpex-actions"><XpexQuickAction icon={GraduationCap} title="Nova turma"/><XpexQuickAction icon={BookOpen} title="Novo curso" disabled={false} href={xpexCourseStudioRoute(organizationSlug)} detail="Abrir Course Studio"/><XpexQuickAction icon={Users} title="Novo aluno"/><XpexQuickAction icon={FilePlus2} title="Relatório"/></div></section>
