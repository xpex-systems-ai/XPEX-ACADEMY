...
function PoleDashboard({ organizationName, organizationSlug, data, failed }: { organizationName?: string; organizationSlug: string; data?: XpexLaunchReadinessData | null; failed: boolean }) {
  ...
  return <>
    ...
    <div className="grid gap-5 lg:grid-cols-2"><XpexPanel><XpexSectionHeader eyebrow="Turmas ativas" title="Acompanhamento de turmas"/><XpexEmptyState compact title="Resumo de turmas em breve" description="Consulte as turmas persistidas pela área nativa. Este resumo ainda não está integrado."/></XpexPanel><XpexPanel><XpexSectionHeader eyebrow="Atividade e agenda" title="Operação recente"/><XpexEmptyState compact title="Operação ainda sem dados" description="Eventos, avisos e ações auditáveis aparecerão quando houver uma fonte persistida disponível."/></XpexPanel></div>
