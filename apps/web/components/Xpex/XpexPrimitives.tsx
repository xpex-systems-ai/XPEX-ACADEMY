...
export function XpexQuickAction({ icon: Icon, title, disabled = true, href, detail }: { icon: LucideIcon; title: string; disabled?: boolean; href?: string; detail?: string }) {
  const content = <><Icon aria-hidden="true" size={20}/><strong>{title}</strong>{disabled ? <small>Em breve</small> : detail ? <small>{detail}</small> : null}</>
  if (!disabled && href) return <Link href={href} className="xpex-quick-action">{content}</Link>
  return <button type="button" disabled={disabled} className="xpex-quick-action">{content}</button>
}
