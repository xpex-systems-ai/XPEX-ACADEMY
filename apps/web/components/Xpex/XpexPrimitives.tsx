...
export function XpexCourseCard({ title, href, progress }: { title: string; href: string; progress: number | null }) { return <article className="xpex-card p-5"><p className="xpex-label">Curso matriculado</p><h3 className="mt-2 text-lg font-black text-white">{title}</h3>{progress === null ? <p className="mt-3 text-sm text-slate-400">Progresso indisponível</p> : <div className="mt-4"><XpexProgressBar value={progress} label={`Progresso de ${title}`} /></div>}<a href={href} className="xpex-primary">Continuar aprendendo</a></article> }
...
export function XpexQuickAction({ icon: Icon, title, disabled = true }: { icon: LucideIcon; title: string; disabled?: boolean }) { return <button type="button" disabled={disabled} className="xpex-quick-action"><Icon aria-hidden="true" size={20}/><strong>{title}</strong>{disabled && <small>Em breve</small>}</button> }
