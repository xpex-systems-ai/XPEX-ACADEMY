...
const discoveryRows = [
  {
    ...
    items: [
      ...
      { title: 'RAG e conhecimento privado', eyebrow: 'XPeX Core', description: 'Use conhecimento autorizado da sua jornada para estudar com contexto.', icon: BrainCircuit, href: '#gx-copilot', action: 'Abrir bancada' },
  ...
  },
  {
    ...
    items: [
      ...
      { title: 'Boards', eyebrow: 'XPeX Workspace', description: 'Planejamento colaborativo será liberado aqui quando a interface XPeX estiver concluída.', icon: Workflow, href: null, action: 'Em integração' },
      { title: 'Library', eyebrow: 'XPeX Workspace', description: 'Fontes e materiais serão organizados aqui sem sair da experiência XPeX.', icon: LibraryBig, href: null, action: 'Em integração' },
    ],
  },
  {
    ...
    items: [
      { title: 'Trilhas profissionais', eyebrow: 'XPeX Academy', description: 'Combine o curso atual com jornadas profissionais e direção orientada pelo GX.', icon: Route, href: '/xpex/trails', action: 'Explorar trilhas' },
      { title: 'Comunidade', eyebrow: 'XPeX Comunidade', description: 'Compartilhe dúvidas, projetos e aprendizados no ambiente da sua organização.', icon: Users, href: '/xpex/community', action: 'Abrir comunidade' },
...
export default async function XpexAiLabPage() {
  ...
  const gxRecommendation = progress < 25
    ...
    : progress < 70
      ? 'Leve o que já aprendeu para um projeto no Workspace GX e registre decisões e fontes na sua jornada.'
    ...
    >
      <section className="xpex-native-page pb-14">
        <header className="sr-only">
          ...
          <p>Experiência profissional de prática, projetos e mentoria construída sobre o motor acadêmico da XPeX.</p>
        </header>
        ...
        <section className="relative -mx-2 overflow-hidden rounded-[28px] border border-white/10 bg-[#050a12] px-6 py-9 md:px-9 lg:min-h-[430px] lg:px-12 lg:py-12">
          ...
          <div className="relative z-10 max-w-3xl">
            <p className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.24em] text-orange-400"><Bot size={16}/> XPeX AI Lab Studio</p>
        ...
        <section className="mt-11 grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
          <article className="xpex-card border border-cyan-400/20">
            ...
            <h2 className="mt-2 text-2xl font-black">Um estúdio sobre o motor acadêmico XPeX.</h2>
            <div className="mt-5 grid gap-4 md:grid-cols-3">
              ...
              <div className="rounded-2xl border border-white/10 bg-black/15 p-4"><Database className="text-cyan-400" size={20}/><h3 className="mt-3 font-black">Cursos + Trilhas + Progresso</h3><p className="mt-2 text-sm text-slate-400">Cursos, matrícula e progresso seguem como fonte de verdade.</p></div>
              <div className="rounded-2xl border border-white/10 bg-black/15 p-4"><FolderKanban className="text-cyan-400" size={20}/><h3 className="mt-3 font-black">Projetos + Biblioteca</h3><p className="mt-2 text-sm text-slate-400">Projetos e evidências serão apresentados sempre dentro da experiência XPeX.</p></div>
            </div>
          </article>
        ...
        <section id="gx-copilot" className="mt-12 scroll-mt-24" aria-labelledby="gx-copilot-title">
          <div className="xpex-card border border-cyan-400/20 p-6 md:p-8">
            <p className="xpex-label">Bancada central</p>
            <h2 id="gx-copilot-title" className="mt-2 text-3xl font-black">GX Course Copilot</h2>
            <p className="mt-3 max-w-4xl leading-7 text-slate-300">A camada de conversa e RAG está sendo encapsulada no design XPeX. Enquanto a interface final é concluída, nenhum painel técnico externo será aberto para o aluno.</p>
            <div className="mt-5 inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-bold text-slate-400"><BrainCircuit size={17}/> Interface GX em integração</div>
          </div>
        </section>
      </section>
    </XpexAuthenticatedShell>
