...
export default async function ActivityPage({ params }: { params: Promise<{ courseId: string; activityId: string }> }) {
  ...
  const playerActivity = activity.content?.paid_access === false
    ...
    : activity
  return <XpexAuthenticatedShell role="aluno" allowedRoles={['aluno']} displayName={learning.displayName} organizationSlug={learning.organization.slug} poloBranding={learning.branding}><Player key={activityId} courseId={courseId} courseUuid={course.course_id} orgUuid={course.org_uuid} orgSlug={learning.organization.slug} activity={playerActivity} activityMeta={course.activities[index]} previous={course.activities[index - 1]} next={course.activities[index + 1]} lessonNumber={index + 1} totalLessons={course.total_lessons || course.activities.length} completedLessons={course.completed_lessons} progressPercent={course.progress_percent} /></XpexAuthenticatedShell>
}
