...
function OpenSignUpComponent({ org: propOrg }: OpenSignUpComponentProps = {}) {
  ...
  return (
    <div className="w-full max-w-[420px] py-10">
      {/* Header */}
      <h1 className="text-[28px] md:text-[32px] font-black text-white tracking-tight leading-tight">{t('auth.create_account')}</h1>
      <p className="mt-2 text-slate-300 text-[15px] font-medium">{t('auth.fill_in_details')}</p>

      <div className="mt-8">
        {/* Error/Success Messages */}
        {error && (
          <div className="flex justify-center bg-red-950/70 rounded-xl text-red-100 space-x-2 items-center p-4 mb-6 border border-red-500/40">
        ...
        {message && message.email_verified === false && (
          <div className="flex flex-col gap-4 bg-emerald-950/70 rounded-xl text-emerald-100 p-4 mb-6 border border-emerald-500/40">
            ...
            {resendState === 'sent' ? (
              ...
              <button
                ...
                className="text-xs font-semibold text-emerald-100 hover:underline disabled:opacity-50 text-left w-fit"
              >
                {resendState === 'sending'
        ...
        <FormLayout onSubmit={formik.handleSubmit}>
          ...
          <div className="flex flex-row space-x-2">
            <FormField name="first_name">
              <div className="flex items-center space-x-2 mb-1.5">
                <Form.Label className="grow text-[13px] font-semibold text-slate-200">{t('user.first_name')}</Form.Label>
            ...
            <FormField name="last_name">
              <div className="flex items-center space-x-2 mb-1.5">
                <Form.Label className="grow text-[13px] font-semibold text-slate-200">{t('user.last_name')}</Form.Label>
          ...
          <FormField name="password">
            <div className="flex items-center space-x-2 mb-1.5">
              <Form.Label className="grow text-[13px] font-semibold text-slate-200">{t('auth.password')}</Form.Label>
          ...
          <FormField name="username">
            <div className="flex items-center space-x-2 mb-1.5">
              <Form.Label className="grow text-[13px] font-semibold text-slate-200">{t('user.username')}</Form.Label>
          ...
          <FormField name="bio">
            <div className="flex items-center space-x-2 mb-1.5">
              <Form.Label className="grow text-[13px] font-semibold text-slate-200">{`${t('user.bio')} (${t('common.optional')})`}</Form.Label>
            </div>
            <Form.Control asChild>
              <textarea
                onChange={formik.handleChange}
                ...
                className="box-border w-full bg-[#111C2E] text-white rounded-lg px-4 py-3 border border-white/15 appearance-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#00D4FF]/30 focus:border-[#00D4FF] transition-all motion-reduce:transition-none placeholder:text-slate-500 text-sm resize-none min-h-[80px]"
              />
            </Form.Control>
          </FormField>
        ...
        <div className="relative my-6">
          ...
          <div className="relative flex justify-center text-sm">
            <span className="px-3 text-slate-400 bg-[#0B1220] text-xs font-medium">{t('common.or')}</span>
          </div>
        </div>
        ...
        <button
          ...
          className="flex justify-center items-center w-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#00D4FF] focus-visible:ring-offset-2 focus-visible:ring-offset-[#0B1220] bg-[#111C2E] hover:bg-[#17243a] text-white space-x-3 font-medium p-3 rounded-lg border border-white/15 transition-all motion-reduce:transition-none text-sm disabled:opacity-50"
        >
