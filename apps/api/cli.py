...
def xpex_pilot_bootstrap():
    ...
    async def run():
        ...
        engine = create_async_engine(
            _to_async_url(config.database_config.sql_connection_string),
            pool_pre_ping=True,
        )  # type: ignore
        try:
            accounts = [
                PilotAccount(
                    role,
                    os.environ[f"XPEX_PILOT_{prefix}_USERNAME"],
    ...
    try:
        async with AsyncSession(
            ...
            if short:
                ...
                if not password:
                    print(
                        "❌ Error: LEARNHOUSE_INITIAL_ADMIN_PASSWORD environment "
                        "variable is required"
                    )
                    print(
                        "Please set LEARNHOUSE_INITIAL_ADMIN_PASSWORD environment "
                        "variable before running installation."
                    )
                    raise typer.Exit(code=1)
                print(
                    "Using password from LEARNHOUSE_INITIAL_ADMIN_PASSWORD "
                    "environment variable"
                )
                if email != "admin@school.dev":
                    print(
                        "Using email from LEARNHOUSE_INITIAL_ADMIN_EMAIL "
                        f"environment variable: {email}"
                    )
...
def backfill_faststart(
    ...
    dry_run: Annotated[
        bool, typer.Option(help="Only report what would change")
    ] = False,
    limit: Annotated[
        int, typer.Option(help="Max files to process (0 = no limit)")
    ] = 0,
):
    ...
    bounded = limit if limit and limit > 0 else 0
    ...
    try:
        ...
        for page in paginator.paginate(Bucket=bucket, Prefix=prefix):
            for obj in page.get("Contents", []):
                ...
                try:
                    head = s3.get_object(
                        Bucket=bucket, Key=key, Range="bytes=0-2097151"
                    )
                    body = head["Body"]
                    try:
                    ...
                    print(f"  ⚠️  {key}: could not read head ({error})")
                    failed += 1
                    continue
        ...
        print(f"⚠️  scan aborted early: {error}")
...
def transcode_backfill(
    limit: Annotated[
        int, typer.Option(help="Max activities to process (0 = all)")
    ] = 0,
    inline: Annotated[
        bool, typer.Option(help="Transcode inline now instead of enqueuing")
    ] = False,
):
