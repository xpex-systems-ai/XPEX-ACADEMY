...
async def mercadopago_webhook(
    ...
    if not secret:
        raise HTTPException(
            ...
            detail="Mercado Pago webhook is not configured",
        )

    x_signature = request.headers.get("x-signature", "")
    x_request_id = request.headers.get("x-request-id", "")
    data_id = request.query_params.get("data.id", "") or request.query_params.get(
        "data_id", ""
    )
    if not verify_webhook_signature(x_signature, x_request_id, data_id, secret):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Invalid Mercado Pago webhook signature",
        )
