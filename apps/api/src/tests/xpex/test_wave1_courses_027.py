...
async def test_wave1_factory_persists_complete_private_curricula(session) -> None:
    ...
    assert all(
        lesson.learning_objective and lesson.lesson_script and lesson.summary for lesson in lessons
    )
    assert all(
        lesson.exercise_json and lesson.practical_activity_json and lesson.completion_criteria_json
...
async def test_exactly_one_canary_is_created_for_ai_zero(session) -> None:
    ...
    lesson = (
        await session.execute(select(XPeXLesson).where(XPeXLesson.id == first.lesson_id))
    ).scalars().one()
    assert len(jobs) == 1
...
async def test_assessment_scores_are_persisted_and_retries_are_bounded(session) -> None:
    await seed_official_catalog(session)
    await seed_wave1_courses(session)
    first = await submit_assessment(
        session,
        assessment_key="XPEX-S02-C01-A-FINAL",
...
def test_media_pipeline_requires_complete_qa_and_stops_at_human_gate() -> None:
    artifact = {
        "uri": "private/canary.mp4",
        "checksum_sha256": "a" * 64,
        "mime_type": "video/mp4",
        "duration_seconds": 90,
        "captions_uri": "private/canary.vtt",
    }
    qa = {
        "file_valid": True,
        "audio_valid": True,
