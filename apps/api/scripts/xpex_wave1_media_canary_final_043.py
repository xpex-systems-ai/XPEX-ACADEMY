...
Mission XPEX-WAVE1-MEDIA-CANARY-FINAL-043 resumes the already-certified Wave 1 canary
...
MISSION_ID = "XPEX-WAVE1-MEDIA-CANARY-FINAL-043"
...
_SECRET = re.compile(r"(?i)(bearer\s+|hf_)[A-Za-z0-9._-]+")


def _provider_submission_available(qa: dict[str, Any]) -> bool:
    return int(qa.get("provider_submission_count", 0)) == 0 and not qa.get(
        "provider_request_id"
    )


def _begin_provider_submission(qa: dict[str, Any]) -> dict[str, Any]:
    if not _provider_submission_available(qa):
        raise RuntimeError("STOP: Fal provider submission budget exhausted")
    return {
        **qa,
        "provider_submission_count": 1,
        "provider_submission_started_at": datetime.now(UTC).isoformat(),
        "provider_call": True,
...
def _safe_error(exc: Exception) -> str:
    return _SECRET.sub("[REDACTED]", " ".join(str(exc).split())[:2000])


async def _artifact_valid(uri: str, checksum: str, suffix: str) -> bool:
    try:
        with tempfile.TemporaryDirectory(prefix="xpex-rehydrate-check-") as directory:
            path = await asyncio.to_thread(
                materialize_storage_key, uri, str(Path(directory) / f"artifact{suffix}")
            )
            return Path(path).stat().st_size > 0 and _sha256_file(path) == checksum
    except Exception:  # noqa: BLE001 -- missing old ephemeral artifacts are expected here
        return False
...
async def _claim(session: AsyncSession) -> tuple[XPeXWaveMediaJob, XPeXLesson] | None:
    ...
    if job.status == "AVATAR_OR_VISUAL_RENDER":
        if not _provider_submission_available(qa):
            print(f"{MISSION_ID} BLOCKED job_id={job.job_id} provider_budget_exhausted=true")
            return None
...
async def run(*, execute: bool) -> int:
    ...
    try:
        async with AsyncSession(engine, expire_on_commit=False) as session:
            ...
            async def before_video_submit() -> None:
                qa = dict(job.qa_json or {})
                job.qa_json = _begin_provider_submission(qa)
                session.add(job)
                await session.commit()

            async def after_video_submit(request_id: str) -> None:
                job.provider_job_id = request_id
                job.qa_json = {**(job.qa_json or {}), "provider_request_id": request_id}
                session.add(job)
                await session.commit()
            ...
            source = VideoLessonSource(
                ...
                registry=registry,
                before_video_submit=before_video_submit,
                after_video_submit=after_video_submit,
            )
            ...
            try:
                if stage == "AVATAR_OR_VISUAL_RENDER":
                    # The 041 evidence proves the old paths pre-date this container.
                    # Rebuild only missing prerequisites; these handlers cannot call
                    # the motion-video provider.
                    ...
                    narration_ok = bool(
                        manifest.narration
                        and await _artifact_valid(
                            manifest.narration.uri,
                            manifest.narration.checksum_sha256,
                            ".wav",
                        )
                    )
                    if not narration_ok:
                        manifest = await handlers.narrating(manifest)
                    visual = manifest.assets[0] if manifest.assets else None
                    visual_ok = bool(
                        visual
                        and await _artifact_valid(visual.uri, visual.checksum_sha256, ".png")
                    )
                    if not visual_ok:
                    ...
                    job.qa_json = {
                        **(job.qa_json or {}),
                        ...
                        "recovery_attempt_count": int((job.qa_json or {}).get("recovery_attempt_count", 0)) + 1,
                    }
                    session.add(job)
                    await session.commit()
