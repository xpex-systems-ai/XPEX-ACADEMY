...
|---|---|---|---|---|---|---|
| 1 | MISSION-009 — Audit/Webhook Reliability Discovery | Map real webhook delivery runtime, retry behavior, event publishers and audit gaps. | Current webhook DB/router/services and observability docs. | High if it uncovers missing guarantees. | No code changes; documented delivery/audit evidence and ADR recommendation. | Remove docs/ADR from the mission. |
| 2 | MISSION-010 — Platform Core Security Contract | Create checklist for tenant/RBAC requirements in all new routes/services. | This registry, RBAC helpers, org_auth. | Medium: may require future validator scope. | Checklist references real helpers and prohibits parallel authz. | Revert checklist. |
