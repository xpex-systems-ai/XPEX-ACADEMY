...
| LEARNHOUSE_PORT | API | runtime_configuration | Yes for current container port | API Uvicorn port | No | No | Current entrypoint consumes this with fallback to 9000 | For Option A set to 9000 and route provider traffic to 9000; for Option B keep as fallback only |
| PORT | Railway/API provider | runtime_configuration | Expected by many providers but not currently consumed by API entrypoint | Provider port binding | No | No | Ignored by current API entrypoint, creating Railway readiness risk | Do not rely on `PORT` until API Container Port and Bind Hardening is completed |
| COLLAB_PORT | Collab | runtime_configuration | Yes for Collab service | Collab listener port | No | No | Defaults to 4000 in source/container | Set per Railway Collab service if deployed |
| Web runtime port | Web | runtime_configuration | Platform-owned | Vercel/Next runtime binding | No | No | Vercel manages request routing; Docker uses `PORT=3000` only for container mode | Do not mix with API/Collab port contract |
